...
enhance = Blueprint('enhance', __name__)

# 增强前的位置
abspath = 'c:\\zed\\image-enhance\\data_set_img\\'
# 增强后保存的位置
result = 'c:\\zed\\image-enhance\\result\\'
# 哪张图片
img = 'a.jpg'

@enhance.route('/', methods=['POST'])
def images_enhance():
    imgShift(abspath+img)
    # imgResize(abspath+img)
    return '图像增强'
...
def imgShift(pic):
    ...
    # plt.axis('off') # 不用坐标系
    # ax2.set_title("deal")
    plt.savefig(result + img, format='jpg')
...
def imgResize(pic):
    ...
    # plt.savefig('./result/' + 'b.jpg', format='jpg')
    plt.savefig(result + img, format='jpg')
